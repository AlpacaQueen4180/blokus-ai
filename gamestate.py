...
class Board:
    ...
    def adj(self, player_id, placement):
        ...
        for x, y in placement:
            if self.in_bounds((x + 1, y)) : adjacents += [self.state[y][x + 1] == player_id]
            if self.in_bounds((x - 1, y)) : adjacents += [self.state[y][x - 1] == player_id]
            if self.in_bounds((x, y - 1)) : adjacents += [self.state[y - 1][x] == player_id]
            if self.in_bounds((x, y + 1)) : adjacents += [self.state[y + 1][x] == player_id]
        return True in adjacents
    def corner(self, player_id, placement):
        corners = []
        for x, y in placement:
            if self.in_bounds((x + 1, y + 1)) : corners += [self.state[y + 1][x + 1] == player_id]
            if self.in_bounds((x - 1, y - 1)) : corners += [self.state[y - 1][x - 1] == player_id]
            if self.in_bounds((x + 1, y - 1)) : corners += [self.state[y - 1][x + 1] == player_id]
            if self.in_bounds((x - 1, y + 1)) : corners += [self.state[y + 1][x - 1] == player_id]
    ...
    def update_player(self, piece, board) :
        ...
        for c in piece.corners: # Add the player's available corners
            if board.in_bounds(c) and not board.overlap([c]):
                self.corners.add(c)
    def getpossiblemoves(self, pieces, game): # Updates the corners of the player, in case the corners have been covered by another player's pieces.
        self.corners = set([(x, y) for (x, y) in self.corners if game.board.state[y][x] == '_'])
        ...
        for cr in self.corners:          # Check every available corner
            for sh in pieces:            # Check every available piece
                for num in range(sh.size):                          # Check every reference point the piece could have
                    for flip in ["h", "v"]:                         # Check every flip
                        for rot in [0, 90, 180, 270]:               # Check every rotation
                            candidate = copy.deepcopy(sh)           # Create a copy to prevent an overwrite on the original
                            candidate.create(num, cr)
                            candidate.flip(flip)
                            candidate.rotate(rot)
                            if game.valid_move(self, candidate.points): # If the placement is valid and new
